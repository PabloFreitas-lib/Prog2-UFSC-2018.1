...
lista_t *organiza_lista(lista_t *l,int a, int b){
	void *aux;
	void *posicao;
	celula_t *c =l->cabeca;
	if(a>0 && b>0){
	celula_t *c =l->cabeca;
	for(int i =0 ; i< a;i++)c= c->prox; // coloquei na poscao da lista que a celula 'a' se encontra
	pilha_desempilha(lista_dado(c),&aux); // subtituir esse c->dado por alguma coisa
	pilha_topo(lista_dado(c),&posicao);
	lista_insere_posicao(l,*(const unsigned int*)posicao,aux); // c->dado eh uma pilha depois acessar a posicao da pilha que ela se encontra no conteudo da pilha
	while(c->prox->dado != NULL){
		pilha_desempilha(lista_dado(c),&aux);
		pilha_topo(lista_dado(c),&posicao);
			lista_insere_posicao(l,*(const unsigned int*)posicao,aux);
	}
	here_b:
		for(int i =0 ; i< b;i++)c= c->prox; // coloquei na poscao da lista que a celula 'a' se encontra
		pilha_desempilha(lista_dado(c),&aux); // subtituir esse c->dado por alguma coisa
		pilha_topo(lista_dado(c),&posicao);
			lista_insere_posicao(l,*(const unsigned int*)posicao,aux); // c->dado eh uma pilha depois acessar a posicao da pilha que ela se encontra no conteudo da pilha
		while(lista_dado(c->prox) !=NULL){
			pilha_desempilha(lista_dado(c),&aux);
			pilha_topo(lista_dado(c),&posicao);
				lista_insere_posicao(l,*(const unsigned int*)posicao,aux);
		}
...
celula_t *set(lista_t *l,void *elem){
	return(lista_busca_recursiva(l,elem,lista_cabeca(l)));
}
